...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      #tmp = tf.split(0, self.config.batch_size, embed)
      inputs = embed
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection'):
        U = tf.get_variable('U', initializer = tf.random_normal( (self.config.hidden_size, len(self.vocab) )) )
        b_2 = tf.get_variable('b_2', initializer = tf.random_normal ( (len(self.vocab),) ))
    for o in rnn_outputs:
        tmp = tf.matmul(o, U) + b_2
        outputs.append(tmp)
