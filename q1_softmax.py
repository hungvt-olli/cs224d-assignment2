...
def softmax(x):
  ...
  xm = tf.reduce_max(x, reduction_indices=1, keep_dims=True)    
  x = tf.sub(x, xm)    
  ex = tf.exp(x)
  sex = tf.reduce_sum(ex, 1)
  rs = tf.expand_dims(sex, 1)
  out = tf.div(ex, rs)    
...
def cross_entropy_loss(y, yhat):
  ...
  #with tf.Session():
  logyhat = tf.log(yhat)
  #print("logyhat", logyhat.eval())
    
  yyt = tf.mul(tf.to_float(y), logyhat)
  #print("yyt", yyt.eval())
    
  tout = -tf.reduce_sum(yyt)
  out = tout#.eval()
  #  print(out.eval())
