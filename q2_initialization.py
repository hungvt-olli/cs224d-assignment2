...
def xavier_weight_init():
  ...
  def _xavier_initializer(shape, **kwargs):
    ...
    """
    ### YOUR CODE HERE
    # out = tf.get_variable(shape=shape, initializer=tf.contrib.layers.xavier_initializer())
    ee = ( 6 / sum(shape)) ** 0.5
    out = tf.random_uniform(shape=shape, minval=-ee, maxval=ee, dtype=tf.float32)
